...
$stmt = $conn->prepare("SELECT cos_dtr.intern_absent,cos_dtr.date_from, 
cos_dtr.date_to,cos_dtr.dtr_id,tbl_users.cos_company,tbl_users.first_name, 
tbl_users.status,tbl_users.last_name, cos_dtr.rendered_hours, 
cos_dtr.date_submitted,cos_dtr.dtr_status,tbl_users.stud_company 
FROM tbl_users INNER JOIN cos_dtr 
ON cos_dtr.stud_id=tbl_users.member_no
WHERE cos_company = 1 
AND status = 0 AND dtr_status ='Pending'");
$stmt->execute();
...
<td><center><?php echo $row['first_name'];?> 
<?php echo $row['last_name'];?></td>
<td><center><?php echo $row['date_submitted'];?></td>
<td><center><?php echo $row['rendered_hours'];?></td>
<td><center><?php echo $row['intern_absent'];?></td>
<td><center><?php echo $row['date_from'];?> to 
<?php echo $row['date_to'];?></td>
<td><center><a target = "_blank" 
href = "view-attached-dtr.php?id=<?php echo $row['dtr_id']?>" 
class="btn btn-info"><i class="far fa-eye"></i></a>
</td>
<td><center><a onclick = "return confirm('Approve this DTR ?')" 
class="btn btn-success" href="app/approve-dtr.php?id=<?php echo $row['dtr_id']?>">
<i class="far fa-thumbs-up"></i></a>
</td>

<td><center><a onclick = "return confirm('Disapprove this DTR ?')" 
class="btn btn-danger" 
href="app/dis-approve-dtr.php?id=<?php echo $row['dtr_id']?>">
<i class="far fa-thumbs-down"></i></a></td>
...
ON cos_dtr.stud_id=tbl_users.member_no
WHERE cos_company = 1 
AND status = 0 AND dtr_status ='Approved'");
$stmt->execute();
$result = $stmt->fetchAll();
...
2.8.0/Chart.min.js" crossorigin="anonymous"></script>
...
<script src="https://cdn.jsdelivr.net/npm/popper.js@1.12.9/dist/
